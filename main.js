...
class LandingSimulator {
    ...
    setupControls() {
        window.addEventListener('keydown', (event) => {
            if (event.code === 'KeyZ') {
                ...
                shootPosition.add(forward.clone().multiplyScalar(2));
    ...
    restartGame() {
        ...
        this.enemyManager.enemies.forEach(enemy => this.scene.remove(enemy.mesh));
    ...
    animate() {
        ...
        // Spawn de novos inimigos apenas no início de cada fase
        if (this.enemyManager.gameState !== 'landing' && this.enemyManager.enemies.length === 0) {
    ...
    setupCamera() {
        this.cameraOffset = new THREE.Vector3(0, 1.0, -1); // Aumentado o offset
