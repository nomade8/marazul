...
class EnemyManager {
    constructor(scene) {
        ...
        this.enemyShootCooldown = 1500; // Cooldown de tiro do inimigo (ms)
        this.boundarySize = 150; // Aumentado de 60 para 180
        this.changeDirectionInterval = 3000; // Intervalo menor para reavaliar direção
    ...
    createEnemy(playerPosition, spawnAttempt = 0) {
        ...
        this.enemies.push({
            ...
            speed: 0.08 + Math.random() * 0.030 // Reduced speed variation
        });
        ...
        this.enemies.forEach(enemy => {
            ...
            if (now - enemy.lastDirectionChange > this.changeDirectionInterval) {
                ...
                const randomDirection = new THREE.Vector3(
                    Math.random() * 2 * -0.1,
                    Math.random() * 0.2 - 0.1, // Menos variação vertical
                    Math.random() * 2 - 1
                ).normalize();
            ...
            // Update rotation smoothly (mantido)
            const targetPos = enemy.mesh.position.clone().add(enemy.moveDirection);
            enemy.mesh.lookAt(targetPos);
            // Rotação Z removida, lookAt já orienta corretamente
